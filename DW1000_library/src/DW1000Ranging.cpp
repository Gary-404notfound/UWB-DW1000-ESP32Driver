...
void DW1000RangingClass::startAsAnchor(char address[], const byte mode[], const bool randomShortAddress) {
	...
	uint16_t network_id = 0x00;
	do
	{
		DW1000Ranging.configureNetwork(_currentShortAddress[0]*256+_currentShortAddress[1], DEFAULT_NETWORK_ID, mode);
		delay(100);
		network_id = DW1000.getNetworkID();
		if (DEBUG){
			Serial.print("Network ID: ");
			Serial.printf("%02x\r\n", network_id);
		}
	} while (network_id != DEFAULT_NETWORK_ID);
